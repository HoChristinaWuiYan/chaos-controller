...
The `Disruption` spec takes a `duration` field. This field represents the amount of time after the disruption's creation before 
all chaos pods automatically terminate and the disruption stops injecting new ones. This field takes a string representation of a duration, 
such as "45s", "1h30m", etc. Valid time units are "s" (seconds), "m" (minutes), and "h" (hours).
